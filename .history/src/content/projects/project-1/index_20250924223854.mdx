...
```mermaid
flowchart TD
    A[Persona Profile] --> B[Planner Agent -  weekly plan]
    B --> C[Day Planner - hour-by-hour timeline]
    C --> D[POI & Route Grounder - Google Places / Directions]
    D --> E[Validator - opening hours, time budgets, travel time checks]
    E -- ok --> F[Trajectory Exporter - TripLayer JSON]
    E -- violations --> G[Repair Prompt - regenerate failing steps]
    G --> C
    F --> H[Deck.GL UI - React TripLayer]
```
